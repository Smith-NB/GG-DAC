...
function main()
    ...
    formula = Dict("Au" => N) # nanoparticle formula
    formulaString = "" 
    ...
    energyRounding = 3 # dp that energies are stored in clusterVector with
    ...
    function perturber(coords::Matrix{Float64})
        ...
        if r < 0.75 # this is the likelihood of the CDO being used for perturbation
            return DC.perturbCluster(coords, 1.0), "CDO perturbation" # 1.0 is the stepwidth for the CDO
        end
    ...
    hopsToDoPerCluster = 75000 #hops to perform per division.
    ...
    eLim_denominator = 3 # rho value for calculating E_thresh (Equation 3 in paper).
    ...
    probsReduced = DC.gmmposterior(gmm, XReduced)[1] 
    ...
    probsTotal = DC.gmmposterior(gmm, XTotal)[1] 
    # structures assigned to cluster they most likely belong to
    ...
    #=
    clusterEnergyRangesTotal = Vector{Tuple{Float64, Float64}}(undef, nClusters)
    for i in 1:nClusters
        minClusterEnergy = explorationData.vec[clusterIndicesTotal[i][1]].energy
        clusterLength = length(clusterIndicesTotal[i])
        maxClusterEnergy = explorationData.vec[clusterIndicesTotal[i][clusterLength]].energy
        clusterEnergyRangesTotal[i] = (minClusterEnergy, maxClusterEnergy)
    end
    =#
    ...
    # what set of tasks to perform after each optimisation.
    # a set of options is provided in BasinHopping.jl
    postOptimisationTasks = DC.extendedPostOptimisationTasks!
        ...
        for k in 1:nClusters
            ...
            if istaskdone(tasks[k]) && clusterExploitationTimes[k][] < hopsToDoPerCluster
                ...
                tasks[k] = Threads.@spawn begin
                    ...
                    MetC = DC.GMMMetC(gmm, k, pca, :maxProbOnly, true, kT, io)
                    # Reseeder 
