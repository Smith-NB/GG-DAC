...
function perturbClusterSurface(coords::Matrix{Float64}, nAtomsToMove::Number, rCut::1.3549)
	...
	r = getDistancs(coords)
	coordinationNumbers = [count(i->(0.0<i<=rCut), r[:, j]) for j in 1:DAC.getNAtoms(atoms)]
	minCoord = minimum(coordinationNumbers)
	minCoordAtoms = findall(i->i==minCoord, cNum)

	atomsToMove = nothing

	# if the number of low coord atoms is less than nAtomsToMove, only move the fewer amount
	if nAtomsToMove <= length(minCoordAtoms) 
	...
	for i in 1:nAtomsToMove
		coords[atomsToMove[i], :] = sphericalToCartesian(r, sphericalCoords[i, 1], sphericalCoords[i, 2])
	end
