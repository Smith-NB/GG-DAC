...
function LJ(epsilon::Float64, sigma::Float64, rc::Float64, N::Int64)
	LJ(epsilon, sigma, rc, zeros(Float64, 3, N), zeros(Float64, N), zeros(Float64, N), zeros(Float64, N), sigma^6, sigma^12)
end
...
function calculateForces!(atoms::Cluster, calc::LJ)
	...
	for n in 1:natoms-1
		...
		for i in n+1:natoms
			...
			#calc.r2[i] = calc.distance_vectors[1, i]*calc.distance_vectors[1, i] + calc.distance_vectors[2, i]*calc.distance_vectors[2, i] + calc.distance_vectors[3, i]*calc.distance_vectors[3, i]
			r2::Float64 = calc.distance_vectors[1, i]*calc.distance_vectors[1, i] + calc.distance_vectors[2, i]*calc.distance_vectors[2, i] + calc.distance_vectors[3, i]*calc.distance_vectors[3, i]

			dist4::Float64 = r2*r2
			dist8::Float64 = dist4*dist4
			dist14::Float64 = dist8*dist4*r2

			dV::Float64 = -24 * calc.epsilon*(calc.doublesigma12/dist14 - calc.sigma6/dist8)
