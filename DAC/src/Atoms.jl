...
function getRoncagliaAtomClasses()
    # specify classes
    f = """
...
function classifyClusterRoncaglia(ncna::Vector{Dict{Tuple{UInt8, UInt8, UInt8}, UInt16}})

end

classifyClusterRoncaglia(coordinates::Matrix{Float64}, rcut::Float64) = classifyClusterRoncaglia(getNormalCNAProfile(coordinates, rcut))

classifyClusterRoncaglia(cluster::Cluster, rcut::Float64) = classifyClusterRoncaglia(getNormalCNAProfile(getPositions(cluster), rcut))
